...
translate_process :: proc(tcr: Translate_Collect_Result, config: Config, types: Type_List, decls: Decl_List) -> Translate_Process_Result {
	...
	return {
		...
		link_suffix = config.remove_function_suffix,
...
final_type_name :: proc(name: Type_Name, config: Config) -> Type_Name {
	...
	} else if config.force_snake_case_types {
		res = strings.to_snake_case(res)
	} else if config.force_camel_case_types {
		res = strings.to_camel_case(res)
	} else if config.force_pascal_case_types {
		res = strings.to_pascal_case(res)
	}
