...
//@(private="package")
output :: proc(types: Type_List, decls: Decl_List, o: Output_Input, filename: string, footer: string, package_name: string, function_name_case := Output_Name_Case.Original) {
	ensure(filename != "")
	ensure(package_name != "")
	...
	for &d in decls {
		...
		defer strings.builder_destroy(&rhs_builder)
		...
		if kind != current_group.kind ||
			...
			output_group(current_group, o, sb, function_name_case)
	...
	output_group :: proc(g: Output_Group, o: Output_Input, sb: ^strings.Builder, function_name_case: Output_Name_Case) {
		if len(g.decls) == 0 {
			return
		}
		...
		if g.start_foreign_block {
			...
			// link_prefix, and link_suffix is omitted if link_name is used.
			if function_name_case == .Original {
				if o.link_prefix != "" {
					pf(sb, `, link_prefix="%v"`, o.link_prefix)
				}
				if o.link_suffix != "" {
					pf(sb, `, link_suffix="%v"`, o.link_suffix)
				}
			}
		...
		defer delete(group_member_texts)
		...
		for &od, i in g.decls {
			...
			if k == .Proc {
				case_function: #type proc(string, mem.Allocator) -> (string) = nil
				switch function_name_case {
					case .Original: 
						case_function = nil
						pf(&tb, "%v%*s:: %v", d.name, max((longest_name-len(d.name)) + 1, d.explicit_whitespace_after_name), "", rhs)
					case .Ada_Case:
						case_function = proc(s: string, allocator: mem.Allocator) -> (res: string) { return strings.to_ada_case(s, allocator) }
					case .Snake_Case:
						case_function = proc(s: string, allocator: mem.Allocator) -> (res: string) { return strings.to_snake_case(s, allocator) }
					case .Camel_Case:
						case_function = proc(s: string, allocator: mem.Allocator) -> (res: string) { return strings.to_camel_case(s, allocator) }
					case .Pascal_Case:
						case_function = proc(s: string, allocator: mem.Allocator) -> (res: string) { return strings.to_pascal_case(s, allocator) }

				}
				if case_function != nil {
					alias := d.name
					if o.link_prefix != "" {
						alias = strings.trim_prefix(alias, o.link_prefix)
					}
					if o.link_suffix != "" {
						alias = strings.trim_suffix(alias, o.link_suffix)
					}
					alias = case_function(alias, context.allocator)
					defer delete(alias)
					pf(&tb, "@(link_name=\"%v\") %v%*s:: %v", d.name, alias, max(((longest_name * 2 + len("@(link_name=\"\") "))-(len(alias) * 2 + len("@(link_name=\"\") ") )) + 1, d.explicit_whitespace_after_name), "", rhs)
				}
			}
			else do pf(&tb, "%v%*s:: %v", d.name, max((longest_name-len(d.name)) + 1, d.explicit_whitespace_after_name), "", rhs)
