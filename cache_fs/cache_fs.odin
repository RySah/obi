package cache_fs

import "core:os/os2"
import "core:fmt"
import "core:strings"
import hashalgo "core:hash"
import "core:strconv"
import "core:mem"
import "core:path/filepath"
import vmem "core:mem/virtual"

Error :: union #shared_nil {
    os2.Error,
    mem.Allocator_Error
}

File_System :: struct {
    using arena: vmem.Arena,
    path: string,
    children: [dynamic]File_System
}

// This procedure will not destroy its children.
destroy :: proc(system: ^File_System) -> (err: Error) {
    vmem.arena_destroy(&system.arena)
    delete(system.children) or_return
    return nil
}

from :: proc(path: string, allocator := context.allocator) -> (system: File_System, err: Error) {
    context.allocator = allocator
    vmem.arena_init_growing(&system.arena) or_return
    system.children = make([dynamic]File_System) or_return
    system.path = path
    if !os2.exists(system.path) {
        os2.make_directory(system.path) or_return
    }
    return system, nil
}

child :: proc(system: ^File_System, name: string, allocator := context.allocator) -> (child_system: File_System, err: Error) {
    context.allocator = allocator
    path := filepath.join({ system.path, name }, vmem.arena_allocator(&system.arena)) or_return
    child_system = from(path) or_return
    append(&system.children, child_system) or_return
    return child_system, nil
}

clear :: proc(system: ^File_System) -> Error {
    os2.remove_all(system.path) or_return
    os2.make_directory(system.path) or_return
    return nil
}

hashf :: proc(
    allocator: mem.Allocator,
    format: string,
    args: ..any
) -> (output: string, hash_v: u64, err: mem.Allocator_Error) {
    s := fmt.aprintf(format, ..args, allocator=allocator)
    hash_v = hashalgo.murmur64a(transmute([]byte)s)
    return s, hash_v, nil
}

hash :: proc(
    allocator: mem.Allocator,
    args: ..any, 
    sep := " "
) -> (output: string, hash_v: u64, err: mem.Allocator_Error) {
    s := fmt.aprint(..args, sep=sep,allocator=allocator)
    hash_v = hashalgo.murmur64a(transmute([]byte)s)
    return s, hash_v, nil
}

path_from_hash :: proc(system: ^File_System, hash_v: u64) -> (path: string, err: mem.Allocator_Error) {
    hex_hash_buf: [17]byte
    hex_hash := strconv.write_uint(hex_hash_buf[:], hash_v, 16)

    filename := strings.concatenate({ hex_hash, ".txt" }) or_return
    defer delete(filename)

    path = filepath.join({ system.path, filename }, vmem.arena_allocator(&system.arena)) or_return
    return path, nil
}

create_from_hash :: proc(system: ^File_System, hash_v: u64) -> (path: string, err: Error) {
    p := path_from_hash(system, hash_v) or_return
    f := os2.create(p) or_return
    defer os2.close(f)
    return p, nil
}

hash_exists :: proc(system: ^File_System, hash_v: u64) -> bool {
    path, path_err := path_from_hash(system, hash_v)
    if path_err != nil do return false
    return os2.exists(path)
}

existsf :: proc(system: ^File_System, format: string, args: ..any) -> bool {
    s, hash_v, err := hashf(context.allocator, format, ..args)
    defer delete(s)
    if err != nil do return false
    return hash_exists(system, hash_v)
}

exists :: proc(system: ^File_System, args: ..any, sep:=" ") -> bool {
    s, hash_v, err := hash(context.allocator, ..args, sep=sep)
    defer delete(s)
    if err != nil do return false
    return hash_exists(system, hash_v)
}

cachef :: proc(
    system: ^File_System,
    format: string,
    args: ..any
) -> (path: string, err: Error) {
    s, hash_v := hashf(context.allocator, format, ..args) or_return
    defer delete(s)
    
    path = path_from_hash(system, hash_v) or_return

    if !os2.exists(path) do os2.write_entire_file(path, s) or_return

    return path, nil
}

cache :: proc(
    system: ^File_System, 
    args: ..any, 
    sep := " "
) -> (path: string, err: Error) {
    s, hash_v := hash(context.allocator, ..args, sep=sep) or_return
    defer delete(s)
    
    path = path_from_hash(system, hash_v) or_return

    if !os2.exists(path) do os2.write_entire_file(path, s) or_return

    return path, nil
}