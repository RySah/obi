...
c_object_compile_to_subprocess :: proc(
    ctx: ^Build_Context, 
    oc: ^C_Object_Compile, 
...
c_object_compile_to_step :: proc(ctx: ^Build_Context, oc: ^C_Object_Compile, caller_location := #caller_location) -> (step: ^Step, err: Error) {
    _start_trace()
    _trace(caller_location)
    _trace()
    defer if err == nil do _backtrace()

    cmd := c_object_compile_to_subprocess(ctx, oc) or_return
